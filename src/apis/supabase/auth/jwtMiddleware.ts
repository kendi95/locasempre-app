...
export async function jwtMiddleware(token?: string): Promise<ReturnData> {
  try {
    if (token) {
      const { payload } = await decode(token, String(process.env.EXPO_PUBLIC_SECRET_KEY!))
    ...
    if (securedToken) {
      const { payload } = await decode(securedToken, String(process.env.EXPO_PUBLIC_SECRET_KEY!))
