...
export function dateFormat(date: number | Date | undefined, mask: string, utc, gmt) {
...
export const formatTimezone = (date: Date | string) => {
  const dateTimezone = String(date).match(timezone) || ['']
  return dateTimezone?.pop()?.replace(timezoneClip, '').replace(/GMT\+0000/g, 'UTC')
}
...
function getDayName({ y, m, d, _, dayName, short = false }: { y: y() }) {
