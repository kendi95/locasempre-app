...
export function ProfileUserData() {
  ...
  const { control, handleSubmit, formState: { isSubmitting }, setValue } = useForm<FormData>({
  ...
  return (
    <Form key="1" style={{ marginTop: 16, flex: 1 }}>
      {loadData ? (
        <ActivityIndicator size="large" color={theme.colors.title} />
      ) : (
        <>
          <Input.Container label="Nome Completo">
            <Controller 
              control={control}
              name="name"
          ...
          <Input.Container label="E-mail" style={{ width: '100%' }}>
            <Controller 
              control={control}
              name="email"
